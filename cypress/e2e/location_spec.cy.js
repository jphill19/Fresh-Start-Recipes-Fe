describe('Location Page', () => {
    ...
    cy.visit('http://localhost:3001/location', {
      onBeforeLoad(win) {
        cy.stub(win.navigator.geolocation, 'getCurrentPosition').callsFake(
          callback => {
            callback({
              coords: { latitude: 39.757185, longitude: -104.998818 }
            })
          }
        )
      }
  ...
  it.skip('DISPLAYS all content of location component', () => {
    cy.get('.header-section')
      ...
      .should('exist')
      .get('.location-button')
      .should('contain', 'Use Your Location')
      .get('.location-button')
      .should('be.visible')
      ...
      .get('.store-cards-container')
      .should('exist')
      .get('.store-card-wrap#62000115')
      .get('.store-title')
      .should('be.visible')
      .get('.store-title')
      .should('contain', 'King Soopers - Union Station')
      .get('.store-chain')
      .should('be.visible')
      .get('.store-chain')
      .should('contain', 'KINGSOOPERS')
      .get('.store-address')
      .should('be.visible')

      .find('p')
      .eq(0)
      .should('contain', '1950 Chestnut Pl')
      .next()
      .should('contain', 'Denver, CO 80202')
      .get('.store-select-button')
      .should('contain', 'Select Store')
  })
  ...
  it('By default a single location is loaded', () => {
    ...
    cy.get('.store-title')
      .first()
      .should('contain.text', 'King Soopers - Union Station')
    cy.get('.store-address')
      .first()
      .should('contain.text', '1950 Chestnut PlDenver, CO 80202')
    cy.get('.store-address').should('contain.text', 'Denver, CO 80202')
    cy.get('.store-select-button')
      .should('exist')
      .and('contain.text', 'Select Store')
  })
  ...
  it('can handle sad path lcoations that dont have any stores nearby', () => {
    cy.fixture('multiple-locations.json').then(locationData => {
      ...
      cy.get('.no-locations-message')
        .should('exist')
        .and('contain.text', 'No King Soopers nearby your location')
    })
  })
})
...
describe('Navigations', () => {
  it('From home page to location page and selecting store should navigate back home', () => {
    ...
    cy.get('.location-inline-label')
      .should('exist')
      .and('contain.text', 'Union Station')
